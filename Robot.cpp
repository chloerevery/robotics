...
void CRobot::SetPosition(CPointDouble p){ m_pos = p; }
CPointDouble CRobot::Position(){	return m_pos; }
void CRobot::SetPositionCurrent(CPointDouble p){ m_posCurrent = p; }
CPointDouble CRobot::PositionCurrent(){ return m_posCurrent; }
void CRobot::SetPositionPast(CPointDouble p){ m_posPast = p; }
...
{	
	...
	*/
	...
	//로봇 사양
	#define H 0.023                 //(m    ) 질량중심의 높이
	#define B (0.0675/2.)           //(m    ) 구동륜과 캐스터 간 거리
	#define T (0.0675)              //(m    ) 윤거(좌우 바퀴사이 거리)
	#define G (10.0)                //(m/s^2) 중력가속도 9.8이나 10이나 거의 같으므로 관리하기 쉽게 10으로 함. 어차피 정밀한 튜닝은 불가능함
	#define U (0.7)                 //(     ) 마찰계수
	#define M (0.48)                //(kg   ) 로봇 질량
	#define I ((0.25*T)*(0.25*T)*M) //(kgm^2) 로봇 회전 관성 추정치(로봇 중심으로부터 좌 우 50%지점에 질점이 각각 위치해있다고 가정함)

	//AX(1.0) 4.7392176529588763
	#define AX_MAX max_accel       // ((B*U*G)/(B+H*U))*0.8 //가속 시 로봇 진행방향으로 버틸 수 있는 최대 가속도
	#define AX_MIN max_decel       // ((B*U*G)/(B+H*U))*0.6 //감속 시 로봇 진행방향으로 버틸 수 있는 최대 가속도
	#define AY_MAX max_centripetal // (U*G)                 //로봇의 옆방향으로 버틸 수 있는 최대 가속도(로봇의 질량중심이 충분히 낮기때문에 toppling은 무시)
	#define V_MAX   4.             //로봇이 낼 수 있는 속도보다 충분히 큰 값(시뮬레이션에서 특이값을 제한하기 위해서 사용)

	#define MAX_ACCELERATION accel_limit    //(m_userParameter[5]/10)
	#define MAX_VELOCITY     velocity_limit //(m_userParameter[1])

	//PATH
	double c[PATH_POINT_MAX]; //곡률
	double dc[PATH_POINT_MAX];//변곡률(거리에 대한 곡률 차분량)
	double d[PATH_POINT_MAX]; //point간의 거리

	//VELOCITY PROFILE
	double v[PATH_POINT_MAX]; //선속도
